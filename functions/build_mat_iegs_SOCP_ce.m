function p = build_mat_iegs_SOCP_ce(p)
% Building matrices for constraints with MISOCP gas flow relaxation with convex envelope of concave constraints
...
%                    zeros(p.h,1)];
    ...
    % Convex envelope of concave constraints
    A9 = []; b9 = [];
    for jj = 1:p.gn.noN(i)
        j = p.gn.N{i}(jj);
        ...
        flowMin = [];
        for h = 1:p.h
            if p.alphaFixed{i,j}(h,1) == 0
                flowmaxh = 0;
                %flowmaxh = p.gn.phi_max(i,j);
                flowminh = p.gn.phi_max(i,j);
            else
                flowmaxh = p.gn.phi_max(i,j);
                %flowminh = -p.gn.phi_max(i,j);
                flowminh = 0;
            end
            flowMax = [flowMax;flowmaxh];
            flowMin = [flowMin;flowminh];
        end
        
        A9j{jj} = p.m.Sgam{i,j} - diag(flowMax - flowMin)/p.gn.cf(i,j)*p.m.Sphi{i,j};
        b9j{jj} = -flowMax.*flowMin;
