...
np.Gnet = np.v_op^2*1000*np.Gnet/np.scaling;
...
% 
% for i=1:length(tab(:,1))
%     np.Pd(1:p.h,i) = tab(i,2)*ones(p.h,1);
% end
% np.Pd(1:p.h,1) = zeros(p.h,1);   % zero load on the pcc 
hourStart = 12;

np.Pd = np.Pd(:,hourStart:hourStart+p.h);
...
loadratio = table2array(loadratioG);

for i = 1:length(tab(:,1))
    node = find(pairList==tab(i,2));
    for h = 1:p.h
        gn.Gdem(node,h) = tab(i,3)*loadratio(hourStart+h-1,2)/gn.scale;
    end
   
end

%gn.Gdem(1:p.h,1) = zeros(p.h,1);
gn.Gd_max = max(tab(:,3))/gn.scale;
...
gn.d_gu = 0.1412*0.5/mean(1+sum(gn.Gdem(gn.n+1:end,:)));
gn.d_gu_l = 0.001;
