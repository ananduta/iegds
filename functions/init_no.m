function s = init_no(s,p)
% Initialization for network operator; ED IEGDS
% W. Ananduta
% 11/08/2021
...
for i = 1:p.gn.n
    ...
    bco1 = bco1 - p.gn.Gdem(i,:)' - p.tn.Sgu{i}*s.u{i}(:,1);
    
    Aco2 = kron(eye(p.h),[0, ones(1,p.gn.noN(i))]);
    bco2 = p.tn.eps_g*ones(p.h,1);
    bco2 = bco2 + p.gn.Gdem(i,:)' + p.tn.Sgu{i}*s.u{i}(:,1);
