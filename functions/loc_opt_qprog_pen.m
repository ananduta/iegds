function s = loc_opt_qprog_pen(p,s,k,i)
...
%     end
    ...
    for h = 1:p.h
        c_1 = c;
        c_1(p.id_mg) = p.en.d_mg*(s.sigma_mg(h,k)-pmg_i_t(h));
        c_1(p.id_gu) = p.gn.d_gu*(s.sigma_gu(h,k)-dgu_i_t(h));
        c1 = [c1;c_1];
    end 
    ...
    % linear term associated with powerflow constraints
    c4 = zeros(p.h*p.nu(i),1);
    for jj=1:length(p.en.N{i})
        j = p.en.N{i}(jj);
        c4 = c4 + p.m.PF{i,j}{1}'*s.mu_pl{i,j}(:,k) + p.m.PF{j,i}{2}'*s.mu_pl{j,i}(:,k);           
    end
    ...
    for jj=1:length(p.gn.N{i})
        ...
        % linear term associated with penalty of \phi
        cp = cp + p.m.Spen{i,j}'*ones(p.h,1)*p.pen;
    end
    ...
    p.m.c{i} = c1+  c2+  c3+  c4 + c5 + c6 + c7+  c8 + cp;
...
% H = p.m.H{i};
% f = p.m.c{i};
% options = optimset('Display','off','MaxIter',2e6);
...
if res.info.status_val ~= 1 && res.info.status_val ~= -2  && res.info.status_val ~= 2  
    ...
    s.u{i}(:,k+1) = u_i;
        ...
        s.p_ch{i}(:,k+1) = p.m.Sch{i}*u_i;
        s.p_dh{i}(:,k+1) = p.m.Sdh{i}*u_i;
        s.p_mg{i}(:,k+1) = p.m.Smg{i}*u_i;
        s.d_gu{i}(:,k+1) = p.m.Sgu{i}*u_i;
        for jj = 1:length(p.en.N{i})
            j = p.en.N{i}(jj);
            s.p_l{i,j}(:,k+1) = p.m.Spl{i,j}*u_i;
        end
    ...
    %    dim_i = p.h*(3+sum(p.Adj(i,:)));
     %   u_i = u_all(dim_prev_ag+1:dim_prev_ag+dim_i);
        ...
        for jj = 1:length(p.gn.N{i})
            j = p.gn.N{i}(jj);
            s.phi{i,j}(:,k+1) = p.m.Sphi{i,j}*u_i;
            s.pen{i,j}(:,k+1) = p.m.Spen{i,j}*u_i;
        end
