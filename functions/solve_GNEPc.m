...
%% INITIALIZATION
    ...
    for i=1:p.n
        ...
        while 1
            ...
            if mod(k,10) == 0
                k
                 [s.res(k);s.resb(k);s.resc(k);s.res1(k)]
%                   [s.res(k);s.res1(k)]
            end
           ...
           if  s.res(k) < p.er_max && s.res1(k) < p.er_max && s.resb(k) < p.er_max && k > p.t_max
